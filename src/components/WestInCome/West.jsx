  ...
  let categoryPrice = 0;
    ...
    return (<>
        <ul className='westInCome_list'>
            ...
            {isRender('Продукты')?<Products price={foundMoney('Продукты')}/>:''}
            {isRender('Развлечения') ? <Entertainment price={foundMoney('Развлечения')} /> : ''}
            {isRender('Aлкоголь') ? <Alcohol price={foundMoney('Aлкоголь')} /> : ''}
            {isRender('Здоровье')?<Health price={foundMoney('Здоровье')}/>:''}
            {isRender('Транспорт') ? <Transport price={foundMoney('Транспорт')} /> : ''}
            {isRender('Техника') ? <Technique price={foundMoney('Техника')} /> : ''}
            {isRender('Коммуналка')?<Communication price={foundMoney('Коммуналка')}/>:''}
            {isRender('Все для дома') ? <Housing price={foundMoney('Все для дома')} /> : ''}
            {isRender('Спорт') ? <Sport price={foundMoney('Спорт')} /> : ''}
            {isRender('Образование') ? <Education price={foundMoney('Образование')} /> : ''}
